from functions.tensorsom import *
import gzip
...
for ld_id in lds_ids:
    ...
    if not Path(filename).is_file():
        ...
        # Training the SOM
        som_size = int((X_train.shape[0]**0.54321))
        if som_size < 100:
            sigma = 5
        if som_size < 500:
            sigma = 10
        ...
        print("Training: ",end='')
        som.train(X_train)
        to_save=({'X_train':X_train,'y_train':y_train,'X_train_df':X_train_df,'scaler':sc,'trained_model':som.get_trained_model(),'ld_id':ld_id})
    ...
    if not Path(plot_file).is_file():
        print(str(ld_id) + ":Creating som plot file...")
        mapped = som.map_vects(X_train)
        mapped = np.column_stack((mapped,y_train))
    ...
    if not Path(plot_file).is_file():
        ...
        for i in range(X_train.shape[0]-1):
            ...
            distances[i]=np.sqrt(np.sum((X_train[i] - center) ** 2))
        ...
        if target_name == 'ot_all':
            ots = X_train_df[y_train == 1][datetime_name]
        else:
            ots=X_train_df[X_train_df['ot_all']==1][datetime_name]
    ...
    if not Path(result_file).is_file():
        ...
        mappings = np.array(som.map_vects(X_train))
        distance_map=som.distance_map()
        out=np.array(np.where(distance_map>threshold)).T
        df = pd.DataFrame()
        for i in range(out.shape[0]):
            out_rows=X_train_df[(np.equal([out[i,0],out[i,1]],mappings).all(axis=1))]
